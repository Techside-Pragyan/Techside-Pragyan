...
function generateSvg(days, username) {
  ...
  const points = days.map((d, i) => {
    ...
    return { x: Number(x.toFixed(2)), y: Number(y.toFixed(2)), count: d.count, date: d.date };
  });
  ...
  days.forEach((d, i) => {
    ...
    <text x="${p.x}" y="${paddingTop + graphHeight + 22}" text-anchor="middle" fill="#7dcfff" font-size="11" font-weight="500" font-family="'Segoe UI', Ubuntu, -apple-system, BlinkMacSystemFont, Roboto, sans-serif">${label}</text>`;
    }
  });

  // Interactive circle points with tooltips
  let pointsSvg = "";
  points.forEach((p) => {
    pointsSvg += `
  ...
  <style>
    ...
    circle:hover { r: 6px; fill: #bb9af7; stroke: #ffffff; cursor: pointer; transition: all 0.2s ease; }
  </style>
  ...
  <!-- Header -->
  <text x="${paddingLeft}" y="38" class="title">${username}'s Contribution Activity</text>
  <text x="${width - paddingRight}" y="38" text-anchor="end" class="subtitle">Last 31 Days: <tspan class="highlight">${totalMonthContribs} Contributions</tspan></text>
